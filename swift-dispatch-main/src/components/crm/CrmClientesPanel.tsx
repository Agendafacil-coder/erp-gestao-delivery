...
export function CrmClientesPanel({ tenantId }: Props) {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      setCustomers((prev) =>
        prev.map((c) => (c.phone === selectedPhone ? { ...c, name: editName.trim() || null } : c)),
      );
    } catch (e) {
  ...
  const handleSendPromo = async () => {
    ...
    try {
      ...
      toast.success(
        sent === 1 ? "Promoção enviada" : `${sent} mensagem(ns) enviada(s) (máx. 50 por vez)`,
      );
    } finally {
      setSending(false);
    }
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,22rem)_minmax(0,1fr)] xl:grid-cols-[minmax(0,22rem)_minmax(0,1fr)_minmax(0,22rem)]">
            ...
            <>
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <h2 className="text-lg font-semibold truncate">{detail.name || "Cliente"}</h2>
        ...
        <section className="erp-card p-5 space-y-4 lg:col-span-2 xl:col-span-1 min-h-[28rem]">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            Envie texto e imagem para um cliente ou para uma lista (como transmissão). Precisa do
            WhatsApp conectado em Sistema → WhatsApp → Conexão.
          </p>
