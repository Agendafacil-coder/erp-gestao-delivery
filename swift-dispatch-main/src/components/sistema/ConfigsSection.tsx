...
type ConfigTab = "loja" | "operacao" | "equipe";

export function ConfigsSection() {
  const { current, refresh: refreshTenant } = useTenant();
  const [configTab, setConfigTab] = useState<ConfigTab>("loja");
    ...
    <div className="max-w-3xl space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="segmented-control w-full sm:w-auto shrink-0">
          <button
            type="button"
            data-active={configTab === "loja"}
            onClick={() => setConfigTab("loja")}
          ...
          <button
            type="button"
            data-active={configTab === "operacao"}
            onClick={() => setConfigTab("operacao")}
            className="segmented-item text-xs"
          >
            Operação
          </button>
          <button
            type="button"
            data-active={configTab === "equipe"}
            onClick={() => setConfigTab("equipe")}
            className="segmented-item text-xs"
          >
            Equipe
          </button>
